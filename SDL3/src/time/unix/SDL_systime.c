...
bool SDL_TimeToDateTime(SDL_Time ticks, SDL_DateTime *dt, bool localTime)
{
    ...
    if (tm) {
         ...
         * compliant way.
         */
#if defined(SDL_PLATFORM_AMIGAOS3)
        /* AmigaOS 3.x clib2/libnix struct tm has no tm_gmtoff.  Derive the
         * local UTC offset using only ISO C/POSIX time functions instead. */
        if (localTime) {
            struct tm local_tm = *tm;
            struct tm utc_tm;
            struct tm *utc_ptr = gmtime(&tval);

            if (utc_ptr) {
                time_t local_seconds;
                time_t utc_as_local_seconds;

                utc_tm = *utc_ptr;
                /* Let mktime determine DST for the local interpretation of
                 * the UTC broken-down time instead of forcing GMT's tm_isdst. */
                utc_tm.tm_isdst = -1;
                local_seconds = mktime(&local_tm);
                utc_as_local_seconds = mktime(&utc_tm);

                if (local_seconds != (time_t)-1 && utc_as_local_seconds != (time_t)-1) {
                    dt->utc_offset = (int)difftime(local_seconds, utc_as_local_seconds);
                } else {
                    dt->utc_offset = 0;
                }
            } else {
