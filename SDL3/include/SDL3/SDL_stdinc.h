...
/* clib2 wchar.h pulls in a locale_t definition that conflicts with the
   m68k-amigaos system xlocale headers. SDL only needs wchar_t here;
   stddef.h provides that type without dragging in the conflicting locale API. */
#include <stddef.h>
#else
#include <wchar.h>
#endif
...
#if defined(SDL_PLATFORM_AMIGAOS3)
/* The 68k Amiga ABI aligns 32-bit pointers on 16-bit boundaries.
   Therefore { Uint8; void *; } is 6 bytes, not 8 bytes. */
SDL_COMPILE_TIME_ASSERT(struct_alignment, sizeof(SDL_alignment_test) == 6);
#else
