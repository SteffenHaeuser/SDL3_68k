...
#elif defined(SDLNET_AMIGAOS) || defined(SDL_PLATFORM_AMIGAOS) || defined(__amigaos__) || defined(__AMIGA__)
#ifndef SDLNET_AMIGAOS
#define SDLNET_AMIGAOS 1
#endif

#include <exec/types.h>
#include <exec/libraries.h>
#include <proto/exec.h>
/* Avoid duplicate/conflicting clib2 arpa/inet prototypes when using bsdsocket.library directly. */
#ifndef __NO_NET_API
...
/*
 * The Amiga bsdsocket stubs use this global library base.
 * NET_Init()/NET_Quit() own it, matching SDL_net's own init refcount.
 */
...
static int AmigaSocketWrite(int s, const void *buf, size_t count)
{
    return send(s, (const char *) buf, (int) count, 0);
}

static int AmigaSocketRead(int s, void *buf, size_t count)
{
    return recv(s, (char *) buf, (int) count, 0);
}

/*
 * bsdsocket.library has WaitSelect(), not poll(). SDL_net only needs the
 * normal read/write/error readiness bits, so translate its pollfd array.
 */
static int AmigaSocketPoll(struct pollfd *fds, unsigned int nfds, int timeout)
{
    fd_set readfds;
    fd_set writefds;
    fd_set exceptfds;
    ...
    {
        const int rc = WaitSelect(maxfd + 1, &readfds, &writefds, &exceptfds, ptv, NULL);
        if (rc <= 0) {
            return rc;
        }
...
#define read AmigaSocketRead
#define write AmigaSocketWrite
#define poll AmigaSocketPoll

#define INVALID_SOCKET -1
#define SOCKET_ERROR -1
typedef int Socket;
typedef socklen_t SockLen;
typedef struct sockaddr_in AddressStorage;

/*
 * Classic Amiga bsdsocket.library is IPv4-only. clib2 does not expose a
 * usable complete addrinfo/getaddrinfo API here, so provide the small subset
 * SDL_net needs. libnix/NDK already provides struct addrinfo plus the
 * bsdsocket inline getaddrinfo/freeaddrinfo/getnameinfo calls, so do not
 * redefine them there. IPv6 code remains disabled for both CRTs.
 */
#if defined(SDLNET_CLIB2)
struct addrinfo {
    int ai_flags;
    int ai_family;
    int ai_socktype;
...
static void AmigaFreeAddrInfo(struct addrinfo *ai)
{
    while (ai) {
...
static int AmigaGetAddrInfo(const char *node, const char *service, const struct addrinfo *hints, struct addrinfo **res)
{
    struct addrinfo *ai;
    ...
    *res = NULL;
    ...
    ai = (struct addrinfo *) SDL_calloc(1, sizeof (*ai));
    sin = (struct sockaddr_in *) SDL_calloc(1, sizeof (*sin));
    if (!ai || !sin) {
        SDL_free(ai);
        SDL_free(sin);
        return -1;
    ...
    sin->sin_port = service ? htons((Uint16) SDL_atoi(service)) : 0;

    if (!node) {
        sin->sin_addr.s_addr = htonl(INADDR_ANY);
    } else {
        struct hostent *he = gethostbyname(node);
        if (he && (he->h_addrtype == AF_INET) && (he->h_length == (int) sizeof (sin->sin_addr)) && he->h_addr_list && he->h_addr_list[0]) {
            SDL_memcpy(&sin->sin_addr, he->h_addr_list[0], sizeof (sin->sin_addr));
        } else {
            const unsigned long raw = inet_addr(node);
            if ((raw == INADDR_NONE) && (SDL_strcmp(node, "255.255.255.255") != 0)) {
                SDL_free(ai);
                SDL_free(sin);
                return -1;
            }
...
static int AmigaGetNameInfo(const struct sockaddr *sa, socklen_t salen, char *host, size_t hostlen, char *serv, size_t servlen, int flags)
{
    ...
    if (host && hostlen) {
        ip = (const char *) Inet_NtoA(sin->sin_addr.s_addr);
        if (!ip) {
            return -1;
        }
        SDL_strlcpy(host, ip, hostlen);
    }
    if (serv && servlen) {
...
#define getaddrinfo AmigaGetAddrInfo
#define freeaddrinfo AmigaFreeAddrInfo
#define getnameinfo AmigaGetNameInfo
#endif /* SDLNET_CLIB2 */
...
#elif defined(SDLNET_AMIGAOS)
    (void) rc;
    return CreateSocketErrorString(LastSocketError());
#else
...
static int MakeSocketNonblocking(Socket handle)
{
    ...
    #elif defined(SDLNET_AMIGAOS)
    long one = 1;
    return (int) IoctlSocket(handle, FIONBIO, (char *) &one);
    #else
...
static NET_Address *CreateSDLNetAddrFromSockAddr(const struct sockaddr *saddr, SockLen saddrlen)
{
    ...
    if (!saddr || (saddr->sa_family != AF_INET)) {
        SDL_SetError("Only IPv4 addresses are supported on AmigaOS");
        return NULL;
    }
    #endif
...
bool NET_Init(void)
{
    ...
    if (!SocketBase) {
        SDL_SetError("Unable to open bsdsocket.library version 4 or newer");
        SDL_AddAtomicInt(&initialize_count, -1);
        return false;
    }
    #else
...
NET_Address *NET_ResolveHostname(const char *host)
{
    ...
    #ifdef SDLNET_AMIGAOS
    /*
     * bsdsocket.library state is task-local on classic AmigaOS. Do not run
     * name resolution in SDL worker tasks using the caller task's SocketBase.
     * Resolve immediately in the task that called NET_Init().
     */
    SDL_SetAtomicInt(&addr->refcount, 1);
    SDL_SetAtomicInt(&addr->status, (int) ResolveAddress(addr));
    return addr;
    #else
...
#ifdef SDLNET_AMIGAOS
    hints.ai_flags = (!ainfo ? AI_PASSIVE : 0);
#else
...
NET_DatagramSocket *NET_CreateDatagramSocket(NET_Address *addr, Uint16 port, SDL_PropertiesID props)
{
    ...
    for (int i = 0; i < num_handles; i++, ainfo = ainfo->ai_next) {
        ...
        if (sock->allow_broadcast) {
            ...
            #ifdef SDLNET_AMIGAOS
            if (!socket_handle->broadcast && addr) {
                SDL_SetError("Failed to determine IPv4 broadcast address for this interface");
                goto failed;
            }
            #else
            if (!socket_handle->broadcast && (addr || (ainfo->ai_family == AF_INET6))) {
                ...
                setsockopt(handle, IPPROTO_IPV6, IPV6_MULTICAST_IF, (const char *) &ifidx, (SockLen) sizeof (ifidx));
            }
            #endif
        }
    }
