...
#define READ_CHAR(thing)					\
  if (1 != SDL_ReadIO(rw, &tmpchar, 1, 1))  goto badread;	\
  thing = tmpchar;
#define READ_SHORT(thing)					\
  if (1 != SDL_ReadIO(rw, &tmpshort, 2, 1)) goto badread;	\
  thing = SDL_Swap16LE(tmpshort);
#define READ_LONG(thing)					\
  if (1 != SDL_ReadIO(rw, &tmplong, 4, 1))  goto badread;	\
