...
static int dumpstring(SDL_IOStream *rw, Sint32 len, Uint8 type)
{
  ...
  if (!s)
    {
      SDL_SeekIO(rw, len, SDL_IO_SEEK_CUR);/* should I ? */
      return -1;
    }
...
MidiEvent *read_midi_file(MidiSong *song, Sint32 *count, Sint32 *sp)
{
  ...
  if (SDL_memcmp(tmp, "RIFF", 4) == 0) { /* RMID ?? */
    if (SDL_ReadIO(song->rw, tmp, 1, 4) != 4 || SDL_memcmp(tmp, "RMID", 4) != 0 ||
	SDL_ReadIO(song->rw, tmp, 1, 4) != 4 || SDL_memcmp(tmp, "data", 4) != 0 ||
